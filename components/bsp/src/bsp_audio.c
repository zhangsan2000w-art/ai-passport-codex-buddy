...
static bool     s_needs_pre_enable;
...
esp_err_t bsp_audio_set_format(uint32_t hz, uint8_t bits, uint8_t ch) {
    ...
    } else if (s_needs_pre_enable) {
        // bsp_audio_close() 后通道处于 READY；open 内部会先 disable，故先补一次 enable。
        if (s_tx) i2s_channel_enable(s_tx);
        if (s_rx) i2s_channel_enable(s_rx);
        s_needs_pre_enable = false;
    }
